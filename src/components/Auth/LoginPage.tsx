...
export const LoginPage: React.FC<LoginPageProps> = ({ onSuccess }) => {
  ...
  return (
    <div className="min-h-screen w-screen flex flex-col justify-center items-center bg-[#0B132B] p-4 relative overflow-hidden font-sans select-none">
      ...
      <div className="mb-5 inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-800/80 border border-slate-700 backdrop-blur-md text-slate-300 text-xs shadow-lg">
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden relative z-10 transition-all">
        {/* Top Brand Banner */}
        <div className="p-7 pb-5 text-center bg-gradient-to-b from-slate-50 to-white border-b border-slate-100">
          <div 
            className="w-13 h-13 mx-auto rounded-2xl flex items-center justify-center text-white shadow-md mb-2.5 transition-transform hover:scale-105"
          ...
          <p className="text-xs text-slate-500 mt-1">
            Automação Omnichannel Instagram Direct & Facebook Messenger
          </p>
        </div>

        {/* Auth Mode Tabs */}
        <div className="flex border-b border-slate-100 bg-slate-50/80 p-1.5 m-4 mb-2 rounded-xl">
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-5 pt-2 space-y-3.5">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-2.5 rounded-xl text-white font-bold text-xs shadow-md hover:opacity-95 disabled:opacity-50 transition-all flex items-center justify-center gap-2 cursor-pointer mt-3"
            style={{ backgroundColor: primaryColor }}
          >
            {isLoading ? (
              <>
